...
- チャンネル登録: 設定ファイルでの手動管理は行わない。frontendへのGoogleログインをトリガーに、そのアカウントのYouTubeチャンネル登録（サブスクライブ）一覧を取得し`POST /channels`（`backend/src/channelsApiLambda.js`）でDynamoDB（`backend/src/lib/channelsStore.js`。ユーザーのメールアドレスをパーティションキーとし、各ユーザーは自分の行にのみ書き込む）へ永続化する。`discover`Lambda（`backend/src/lambda.js`）はこのDynamoDBのチャンネル一覧を読んで新着検知するため、backend側は継続的なOAuthリフレッシュトークンを保持しない（Issue #93）
- 処理済み動画管理: DynamoDB（`backend/src/lib/dynamoStore.js`。テーブル定義は`backend/serverless.yml`）
- 運用監視（サイレント障害検知、現在無効化中）: Raspberry Piが完全に停止・クラッシュするとAWS側へ一切ログが残らず検知できない問題への対策（Issue #122）。`transcriptApi` LambdaのInvocationsメトリクスに対するCloudWatch Alarm（想定される実行間隔を超えて呼び出しが無ければ異常とみなす、`TreatMissingData: breaching`）→SNS Topic（`OpsAlertTopic`）→`opsAlert` Lambda（`backend/src/opsAlertLambda.js`）経由で、運用監視専用LINE Bot（dev-standards `shared/lambda/opsAlertNotifier.js`・`docs/ops-monitoring-pattern.md`参照）へ通知する設計。ただしデプロイ用IAMユーザーにSNS関連の権限が無く`deploy-backend`が継続的に失敗するため、IAM権限対応まで`backend/serverless.yml`側のリソース定義（`opsAlert`関数・`OpsAlertTopic`・`TranscriptApiNoInvocationsAlarm`）をコメントアウトして一時的に無効化している（Issue #129）。コード自体（`opsAlertLambda.js`等）は残しており、IAM権限対応後はコメントアウトを解除するだけで再度有効化できる

## 秘匿情報

`AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`（必須、デプロイ用）はデプロイに使用する。`YOUTUBE_API_KEY` / `GEMINI_API_KEY`（必須）は動画情報取得・要約生成に使用する。`GOOGLE_OAUTH_CLIENT_ID`（必須）はfrontendのGoogleログインおよび`POST /channels`のアクセストークン検証に使用する（クライアントシークレット・リフレッシュトークンはbackend側では不要）。`LINE_CHANNEL_ACCESS_TOKEN` / `LINE_USER_ID`（任意）は未設定時、LINE通知のみスキップされる。`PI_API_KEY`（必須）は自宅Raspberry PiからのAPI呼び出しを認証する共有シークレットで、Raspberry Pi側にも同じ値を設定する。`OPS_ALERT_LINE_CHANNEL_ACCESS_TOKEN` / `OPS_ALERT_LINE_USER_ID`（任意）は未設定時、運用監視アラートのLINE通知のみスキップされる（ユーザー向け通知用の`LINE_CHANNEL_ACCESS_TOKEN`とは別の、運用監視専用LINE公式アカウント（全プロダクト共通で1つ新規開設したもの、dev-standards `docs/ops-monitoring-pattern.md`参照）の値を登録する）。これらはGitHub Actions Secretsで管理し、コードに埋め込まない。
