...
- 運用監視（サイレント障害検知）: Raspberry Piが完全に停止・クラッシュするとAWS側へ一切ログが残らず検知できない問題への対策（Issue #122）。`transcriptApi` LambdaのInvocationsメトリクスに対するCloudWatch Alarm（想定実行間隔以上呼び出しが無ければ異常とみなす、`TreatMissingData: breaching`）→SNS Topic（`OpsAlertTopic`）→`opsAlert` Lambda（`backend/src/opsAlertLambda.js`）経由で、運用監視専用LINE Bot（dev-standards `shared/lambda/opsAlertNotifier.js`・`docs/ops-monitoring-pattern.md`参照）へ通知する
